...
class VectorDBInt16Global:
    """
    A vector database class using 16-bit (int16) quantization for embeddings,
    with a single global clipping limit. All embeddings are clipped to ±global_limit 
    and scaled to -32767..32767.
    """
    ...
    def __init__(
        ...
        global_limit: float = 1.0,  # Example global limit ±1.0 for int16
        rdict_options=None,
        ...
        self.embed_url = embed_url  # Now added to the constructor
    ...
    def _initialize_faiss_index(self, folder: str, embedding_dim: int):
        """
        Initialize or load a FAISS IndexBinaryIDMap2 for int16-based embeddings.
        """
    ...
    def _generate_embeddings(self, texts: List[str]) -> Dict[str, Dict[str, np.ndarray]]:
        """
        Generate float32 embeddings for each text, then quantize them to int16 via a global ±limit.
        """
        url = self.embed_url
        results = {}
        for text in texts:
            try:
                ...
                # Clip and quantize to int16 using a single global limit
                q_int16 = self._quantize_to_int16(embedding, self.global_limit)
    ...
    @staticmethod
    def _quantize_to_int16(embedding: np.ndarray, limit: float) -> np.ndarray:
        """
        Global-limit int16 quantization:
          1. Clip to ±limit.
          2. scale = 32767 / limit
          3. Round into [-32767..32767]
        """
        clipped = np.clip(embedding, -limit, limit)
        scale = 32767.0 / limit
        scaled = np.round(clipped * scale)
        scaled = np.clip(scaled, -32767, 32767).astype(np.int16)
        return scaled

    @staticmethod
    def _dequantize_int16(emb_int16: np.ndarray, limit: float) -> np.ndarray:
        """
        Convert global-limit int16 back to float32:
          scale = limit / 32767
          float_val = int16_val * scale
        """
        scale = limit / 32767.0
    ...
    def add_documents(self, doc_ids: List[int], docs: List[str], batch_size=64, save=True):
        """
        Add documents in batches, storing int16 embeddings with a single global limit.
        ...
        with tqdm(total=len(docs), desc="Indexing docs (Global Int16)") as pbar:
            for start in range(0, len(docs), batch_size):
                ...
                for doc_id_val, doc_text in zip(batch_ids, batch_docs):
                    self.doc_db[str(doc_id_val)] = {
                        'doc': doc_text,
                        'emb_int16': embeddings[doc_text]['int16']
                    }
    ...
    def search(self, query: str, k=10, binary_oversample=10, compare_float32=False) -> List[Dict]:
        """
        Search with a FAISS binary index, then refine scores using either float32 or int16 embeddings 
        with a global limit.
        """
        ...
        # Phase II: refine with dot product in float32 or global-limit int16
        hits = []
        for hit in initial_hits:
            ...
            if compare_float32:
                ...
                doc_emb = self._dequantize_int16(emb_int16, self.global_limit)
    ...
    def save(self):
        """
        Save FAISS index to disk. RocksDict is updated on each write.
        """
